...
function doGet(e) {
  try {
    ...
    try {
      return HtmlService.createHtmlOutputFromFile('AITeacherPlatform')
        .setTitle('Anh Sao Khue - AI Education Platform')
        .addMetaTag('viewport', 'width=device-width, initial-scale=1.0')
        .setXFrameOptionsMode(HtmlService.XFrameOptionsMode.ALLOWALL);
    } catch (e2) {
      return HtmlService.createHtmlOutput('<h3>Anh Sao Khue AI Platform - Vui long tao file index.html</h3>');
    }
  }
}
...
function showAppDialog() {
  ...
  try {
    ...
    html = HtmlService.createHtmlOutputFromFile('AITeacherPlatform');
  }
  html.setWidth(1280)
    .setHeight(820)
    .setTitle('Anh Sao Khue - AI Education Platform');
